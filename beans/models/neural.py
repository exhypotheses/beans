...
class Neural:
    ...
    def exc(self, input: np.ndarray, output: np.ndarray):
        ...
        with pymc3.Model() as network:
            ...
            weights_in_1 = pymc3.StudentT(name='w_in_1', nu=5, mu=0, sigma=2.5, shape=(input.shape[1], hidden_1),
                                          testval=init_1)

            # Weights from 1st to 2nd layer
            weights_1_2 = pymc3.StudentT(name='w_1_2', nu=5, mu=0, sigma=1.5, shape=(hidden_1, hidden_2),
                                         testval=init_2)
